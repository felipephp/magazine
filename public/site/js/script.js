...
script = {
    ...
    /**
     * Adapt height to screen for containers
     * */
    defaultHeight: function () {
        ...
        $(this.containers).each(function () {
            var h = _this.h - _this.menuHeight;
    ...
    {
        var buttons = $('.anchor-scroll');
        $(buttons).each(function () {
            var id = $(this).attr('href');
            var tag = $(id);
            $(this).on('click', function (e) {
                e.preventDefault();
                var px = tag.offset().top - _this.menuHeight;
                $('html,body').animate({scrollTop: px }, 1300);
            })
        });
        // var tag = $(id);
